...
const Position = () => {
  ...
  useEffect(()=>{
    ...
    const receiveData = async () =>{
        const res = await axiosInstance.get('/hrm/position')
        try{
            if (res){
...
const deleteRecord = async (id) => {
    try{
      const res = await axiosInstance.delete(`/hrm/position?id=${id}`)
            ...
            records.map(item => (
                <div class="card" style={{width: "18rem"}}>
                    <div class="card-body">
                        ...
                        <p class="card-text"><b>Dept has Position: </b>{item.dept_has_position?.dept_name}</p>
                        <p class="card-text"><b>Pos has Salary: </b>{item.posi_has_salary?.net_salary}</p>
